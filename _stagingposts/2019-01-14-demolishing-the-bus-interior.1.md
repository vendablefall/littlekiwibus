...
So we turned to our favourite "shortcut tool", the reciprocating saw. We cut as close to the edge as we could, allowing us to pull out the bulk of the compartments so we could get to the rivets that were holding them in place.
...
We removed the rivets with a drill. The trick is to start with a drill bit slightly smaller than the rivet to make a pilot hole, and then change to a bit the same size as the rivet. My brother (who's nearly more into this project than we are!) advised we should look for a "rivet remover", but when we went to the local Mitre 10 they claimed it didn't exist. We were in a small town and didn't really have many other options so we went back to our drill. 
